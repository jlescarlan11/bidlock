    ...
    <div>
      ...
      {items.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {items.map((bid: any) => (
            <BidCard key={bid.id} bid={bid} userId={userId} hasOtherBids={otherBidListings.has(bid.listings?.id)} />
          ))}
        </div>
      )}
    </div>
...
export default async function BidsPanel({ userId }: { userId: string }) {
  ...
  const won = bidsWithThumbs.filter((b: any) => b.listings?.status === 'ended' && b.listings?.winner_id === userId)
  const lost = bidsWithThumbs.filter((b: any) => b.listings?.status === 'ended' && b.listings?.winner_id !== null && b.listings?.winner_id !== userId)
